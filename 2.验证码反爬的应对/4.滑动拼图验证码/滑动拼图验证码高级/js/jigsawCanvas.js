...
$(function() {
    ...
    function mouseDowns(CoordinateX, CoordinateY) {
        ...
        if(targetblockTimes<1){
            ...
            targetImage.src = 'images/1.png';
            cvas.fillStyle = "rgba(0,0,0,0.6)";
            // 在指定位置绘制38x38大小的矩形
            cvas.fillRect(CoordinateX, CoordinateY, 50, 50); 
            targetblockTimes += 1;
        }else{
    ...
    jigsaw.bind('mouseup', function() {
        ...
        // 误差在2px以内则算成功
        if (Math.abs(endPosition - CoordinateX + missblockFirst) > 15) {
            console.log('验证失败')
            // 缺块和滑块归位
