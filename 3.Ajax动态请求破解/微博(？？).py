...
'''输入关键词到搜索框，完成搜索'''
# 使用selector去定位关键词搜索框
s_input = driver.find_element(By.CSS_SELECTOR, '#search_input')
# 向搜索框中传入字段
s_input.send_keys("努力学习")
# 定位搜索键
confirm_btn = driver.find_element(By.CSS_SELECTOR, '#search_submit')
# 点击
confirm_btn.click()
...
nodes = driver.find_element(By.CSS_SELECTOR, 'div.card > div.card-feed > div.content')
...
for i in range(0, len(nodes), 1):
    ...
    if (flag and nodes[i].find_element(By.CSS_SELECTOR, "p>a[action-type='fl_unfold']").text.startswith('展开全文c')):
...
for page in range(49):
    print(page)
    # 定位下一页按钮
    nextpage_button = driver.find_element(By.CSS_SELECTOR, '下一页')
    # 点击按键
    driver.execute_script("arguments[0].click();", nextpage_button)
    wait = WebDriverWait(driver, 5)
    # 与前面类似
    nodes1 = driver.find_element(By.CSS_SELECTOR, 'div.card > div.card-feed > div.content')
    for i in range(0, len(nodes1), 1):
        ...
        try:
            ...
            flag = False
        if (flag and nodes1[i].find_element(By.CSS_SELECTOR, "p>a[action-type='fl_unfold']").text.startswith('展开全文c')):
